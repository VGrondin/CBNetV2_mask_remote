...
class KeypointsToHeatmaps(object):
    ...
    def msra_generate_target(self, img, joints_3d, sigma):
        ...
        """
        num_joints = 5 # number of keypoints
        ...
        for joint_id in range(num_joints):
            target_weight[joint_id] = joints_3d[joint_id, 2]

            # feat_stride = image_size / [W, H]
            feat_stride = self.dividers   # image_size / heatmap_size
    ...
    def __call__(self, results):
        ...
        if 'gt_keypoints' in results:
            if len(results['gt_keypoints']) > 0:
                ...
                target, target_weight = self.msra_generate_target(target[0],
                                                              tpts[0],
                                                              self.sigma)
                results['heatmaps'] = target
                ...
                hm_sz = results['pad_shape'][0] // self.dividers[0], results[
                    'pad_shape'][1] // self.dividers[1]
                results['heatmaps'] = np.zeros((5, hm_sz[0], hm_sz[1]), dtype=np.float32)
