...
class Resize:
    ...
    def _resize_keypoints(self, results):
        ...
        for key in results.get('keypoint_fields', []):
            if results[key] is None:
                print("Keypoints empty array")
                results[key] = np.array([])
                continue
            if self.keep_ratio:
                # keep aspect ratio constant
                output_scale = np.minimum(results['scale_factor'][0], results['scale_factor'][1])
                scale_x = output_scale
                scale_y = output_scale
            else:
                scale_x = results['scale_factor'][0]
                scale_y = results['scale_factor'][1]
            
            # add one dimension 
            if len(results[key]) == 0:
                print(results[key])
                print("Keypoints empty array, create default one")
                results[key] = np.zeros((2, 5, 3))
                
            results[key][:, :][:, :, 0] = results[key][:, :][:, :, 0] * scale_x
            results[key][:, :][:, :, 1] = results[key][:, :][:, :, 1] * scale_y            
